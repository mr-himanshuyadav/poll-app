...
export default function LiveStudioPage() {
    ...
    return (
        <div className="relative min-h-screen w-full bg-slate-50 text-slate-950 dark:bg-slate-950 dark:text-slate-50">
            <div className="fixed right-4 top-4 z-[60]"><ThemeToggle /></div>
            <LiveStudioHeader session={session} participantCount={totalParticipants} activeParticipantCount={onlineParticipants} onBack={handleBack} onOpenProjector={() => window.open(`/instructor/${session.join_code}/projector`, "_blank", "noopener,noreferrer")} onOpenSettings={() => setSettingsOpen(true)} isUpdating={isSettingsSaving} onPauseSession={handlePauseSession} onResumeSession={handleResumeSession} onEndSession={handleEndSession} />
