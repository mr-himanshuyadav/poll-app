...
export default function LiveStudioPage() {
    ...
    const showResultsToStudents = useCallback(async (question: SessionQuestion, options?: { closeQuestion?: boolean; showOnProjector?: boolean }) => {
        ...
        if (options?.closeQuestion) {
            ...
            await updateSession({ student_display_type: "results", student_question_id: question.id, ...(options?.showOnProjector ? { projector_display_type: "results" as const, projector_question_id: question.id } : { projector_display_type: "waiting" as const, projector_question_id: null }) });
    ...
    const handleRequestShowResults = useCallback((target?: "students" | "both") => {
        // An explicit action must always override the session default. The default is used only when no target was chosen.
        const resolvedTarget = target === "both" || target === "students" ? target : (session?.default_result_visibility === "both" ? "both" : "students");
    ...
    return (
        ...
        if (Object.keys(sessionUpdates).length > 0) await updateSession(sessionUpdates); showNotice("success", "Results are now hidden from students and the projector.", "Results Hidden Everywhere"); }} onShowResultsOnProjector={() => { if (!viewedQuestion) return; if (session.projector_display_type !== "waiting" && session.projector_question_id !== viewedQuestion.id) { setConfirmation({ type: "replace-projector", question: viewedQuestion }); return; } void updateSession({ projector_display_type: "results", projector_question_id: viewedQuestion.id }).then(() => showNotice("success", "Results are now displayed on the projector.", "Projector Updated")); }} onShowResultsOnBoth={() => void handleRequestShowResults("both")} onShowLiveResults={() => { if (!viewedQuestion) return; void updateQuestion(viewedQuestion.id, { results_mode: "live", results_visible: true }).then(() => updateSession({ student_display_type: "results", student_question_id: viewedQuestion.id, projector_display_type: "results", projector_question_id: viewedQuestion.id })).then(() => showNotice("success", "Live results are now updating as participants respond.", "Live Results")); }} onConfirmReplaceLiveQuestion={(question) => setConfirmation({ type: "replace-live", question })} onClearView={() => setViewedQuestionId(null)} onPreviousQuestion={() => {}} onNextQuestion={() => {}} /> : null}
            ...
            {pendingResultsQuestion ? <StudioActionDialog open title={activeQuestion?.id === pendingResultsQuestion.id ? "Show results while live?" : "Close current question?"} description={activeQuestion?.id === pendingResultsQuestion.id ? "Choose whether to keep the question live with instant feedback after each response, or close it and show final results." : "Students are currently answering another question. To show these results, the current live question must be closed."} confirmLabel="Close & Show Results" secondaryLabel={activeQuestion?.id === pendingResultsQuestion.id ? "Show Live Results" : undefined} cancelLabel="Cancel" variant="info" isLoading={areQuestionsSaving} onCancel={() => { if (activeQuestion && activeQuestion.id !== pendingResultsQuestion.id) setViewedQuestionId(activeQuestion.id); setPendingResultsQuestion(null); }} onSecondary={async () => { const question = pendingResultsQuestion; if (activeQuestion?.id !== question.id) return; setPendingResultsQuestion(null); await updateQuestion(question.id, { results_mode: "live", results_visible: true }); await updateSession({ active_question_id: question.id, student_display_type: "question", student_question_id: question.id, ...(pendingResultsTarget === "both" ? { projector_display_type: "results" as const, projector_question_id: question.id } : {}) }); showNotice("success", "Students will now see the answer immediately after responding while the question stays live.", "Live Results Enabled"); }} onConfirm={async () => { const question = pendingResultsQuestion; setPendingResultsQuestion(null); if (activeQuestion?.id === question.id) { await updateQuestion(question.id, { results_mode: "hidden", results_visible: true, status: "closed", closed_at: new Date().toISOString() }); await updateSession({ active_question_id: null, student_display_type: "results", student_question_id: question.id, ...(pendingResultsTarget === "both" ? { projector_display_type: "results" as const, projector_question_id: question.id } : { projector_display_type: "question" as const, projector_question_id: question.id }) }); showNotice("success", "The question was closed and final results are now visible.", "Results Shown"); } else { if (activeQuestion) { await handleSetActiveQuestion(null); setViewedQuestionId(question.id); } await showResultsToStudents(question, { closeQuestion: true, showOnProjector: pendingResultsTarget === "both" }); } }} /> : null}
