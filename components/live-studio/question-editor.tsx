...
export function QuestionEditor({ mode, sessionId, question, isSaving = false, sessionResultsMode = "on_command", onSave, onDelete, onCancel }: QuestionEditorProps) {
    ...
    const applyPreset = (preset: ScalePreset) => updateForm({ scalePreset: preset, scaleMin: preset === "numeric" ? form.scaleMin : 1, scaleMax: preset === "numeric" ? form.scaleMax : 5, scaleLabels: preset === "custom" ? {} : { ...SCALE_PRESETS[preset] } });
